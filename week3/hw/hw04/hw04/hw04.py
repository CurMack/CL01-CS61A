...
def replace_leaf(t, find_value, replace_value):
    ...
    for branch in branches(t):
        new_branch.append(replace_leaf(branch, find_value, replace_value))
...
def preorder(t):
    ...
    if is_leaf(t):
        ...
        prelist = [label(t)]
        for branch in branches(t):
            prelist.extend(preorder(branch))
    return prelist
...
def has_path(t, phrase):
    ...
    if label(t) != phrase[0]:
        ...
        check = False
        for branch in branches(t):
            check = check or has_path(branch, phrase[1:])
        return check
...
def sub_interval(x, y):
    ...
    and any value in y."""
    "*** YOUR CODE HERE ***"
    d1 = upper_bound(x) - lower_bound(y)
...
def div_interval(x, y):
    ...
    reciprocal of y."""
    "*** YOUR CODE HERE ***"
    assert upper_bound(y) * lower_bound(y) > 0, 'no such interval is used as a divisor'
...
def quadratic(x, a, b, c):
    ...
    ext = -b/(2*a)
    if ext >= lower_bound(x) and ext <= upper_bound(x):
        return interval(min(f(lower_bound(x)), f(ext), f(upper_bound(x))), max(f(lower_bound(x)), f(ext), f(upper_bound(x))))
    else:
