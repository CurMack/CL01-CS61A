...
def make_withdraw(balance, password):
    ...
    "*** YOUR CODE HERE ***"
    incorrect = []
    def protected_withdraw(amount, attempt):
        """ In python we can use "fstring", which can insert variables in the string 
        by adding an f before " and puting the variable in {}
        ...
        if len(incorrect) >= 3:
            return f"Too many incorrect attempts. Attempts: {incorrect}"
...
def make_joint(withdraw, old_pass, new_pass):
    ...
    if tmp == 'Incorrect password':
        return tmp
    elif type(tmp) != type(100) and tmp != 'Insufficient funds' :
        return tmp  
