...
def g_iter(n):
    ...
    if n <= 3:
        ...
        g1, g2, g3, i = 1, 2, 3, 4
        while i <= n:
            g1, g2, g3, i = g2, g3,g3+ 2*g2+ 3* g1,i+1
        return g3
...
def missing_digits(n):
    ...
    if n < 10:
        ...
        other, left = n // 10, n % 10
        nxt = other %10
        if left - nxt <= 1:
            return missing_digits(other)
        else:
            return (left - nxt - 1) + missing_digits(other) 
...
def count_change(total):
    ...
    """
    "*** YOUR CODE HERE ***"
    def change_helper(total, maxcoin):
        if total == 0 or maxcoin == 1:
            return 1
        elif total < 0:
            return 0
        else:
            return change_helper(total - maxcoin, maxcoin) + change_helper(total, maxcoin//2)
    
    def max_coin(total):
        if(total == 0):
            return 1
        else:
            return 2 * max_coin(total // 2)
            
    return change_helper(total, max_coin(total))
