...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, beehive,):
        ...
        tmp_range = 0
        while tmp_place != beehive:
            if len(tmp_place.bees) == 0 or tmp_range < self.min_range or tmp_range > self.max_range:
                tmp_place = tmp_place.entrance
                tmp_range += 1
            else:
                return rANTdom_else_none(tmp_place.bees) 
        # REPLACE THIS LINE
        # END Problem 3 and 4
...
class FireAnt(Ant):
    ...
    def reduce_armor(self, amount):
        ...
        # BEGIN Problem 5
        tmp_place = self.place
        tmp_bees = tmp_place.bees[:]
        Ant.reduce_armor(self, amount)
        for bee in tmp_bees:
            Insect.reduce_armor(bee, amount)
        if self.place is not tmp_place:
            tmp_bees = tmp_place.bees[:]
            for bee in tmp_bees:
...
# The WallAnt class
class WallAnt(Ant):
    """WallAnt adds protection to our glorious home base and does nothing each turn"""
    name = 'Wall'
    food_cost = 4
    implemented = True
...
class BodyguardAnt(ContainerAnt):
    ...
    # BEGIN Problem 9
    implemented = True  # Change to True to view in the GUI
    def __init__(self, armer = 2):
        ContainerAnt.__init__(self, armer)
    # END Problem 9
...
# END Problem 13
    ...
    def double_behind(self):
        ...
        while tmp_place != None:
            if tmp_place.ant != None:
                if not tmp_place.ant.doubled:
                    tmp_place.ant.damage *= 2
                    tmp_place.ant.doubled = True;
                if isinstance(tmp_place.ant, ContainerAnt) and tmp_place.ant.contained_ant != None:
                    if not tmp_place.ant.contained_ant.doubled:
                        tmp_place.ant.contained_ant.damage *= 2
                        tmp_place.ant.contained_ant.doubled = True;
    ...
    def reduce_armor(self, amount):
        ...
        # BEGIN Problem 13
        Ant.reduce_armor(self, amount)
        if self.is_true and self.armor <= 0:
            bees_win()
        # END Problem 13
...
def apply_status(status, bee, length):
    ...
    bee.action = status(org_action, bee)
    # END Problem EC
...
class ScaryThrower(ThrowerAnt):
    ...
    def throw_at(self, target):
        # BEGIN Problem EC
        if target and target.direction == 1:
