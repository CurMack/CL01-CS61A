...
def is_bst(t):
    ...
    def bst_max(tr):
        if tr.is_leaf():
            ...
            return max(max(bst_max(branch) for branch in tr.branches), tr.label)
    
    def bst_min(tr):
        if tr.is_leaf():
            return tr.label
        else:
            return min(min(bst_min(branch) for branch in tr.branches), tr.label)
    ...
    if t.is_leaf():
        ...
        if left.label <= t.label:
            return bst_max(left) <= t.label and is_bst(left)
        else:
            return bst_min(right) >= t.label and is_bst(right)
    elif len(t.branches) == 2:
        left, right = t.branches[0], t.branches[1]
        return is_bst(left) and bst_max(left) <= t.label and is_bst(right) and bst_min(right) >= t.label
    else:
        return False
...
def deep_map(f, link):
    ...
    while tmp != Link.empty:
        if type(tmp.first) == Link:
            anstmp.rest = Link(deep_map(f, tmp.first))
        else:
            anstmp.rest = Link(f(tmp.first))
        anstmp = anstmp.rest    
        tmp = tmp.rest
    return ans.rest
